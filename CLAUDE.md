...
- **Background keep-alive**: `ForegroundKeepAlive` (`lib/core/foreground/foreground_keep_alive.dart`, `foregroundKeepAliveProvider`) is the single owner of `ForegroundSessionService.update()`. Cardio calls `setCardio(...)`, the heart-rate panel calls `setHeartRatePanel(...)`; the Android foreground service runs when either needs it, and GPS is only requested by cardio. Never call `ForegroundSessionService` directly from a controller.
