...
- **Shared strap connection**: the connection is shared both ways. `CardioTrackingController.build()` adopts an already-connected `HeartRateService` synchronously (returns `hrConnected: true` and subscribes to readings), its connection-state listener subscribes when the panel connects later, and a successful cardio save re-adopts the connection rather than resetting to disconnected. Device scanning is a stream — `HeartRateService.scanDevices()` yields the growing device list so `HrDevicePickerDialog` lists a strap the moment it is seen; `FakeHeartRateService.emitScanResult()`/`completeScan()` drive it in tests.
- **Background keep-alive**: `ForegroundKeepAlive` (`lib/core/foreground/foreground_keep_alive.dart`, `foregroundKeepAliveProvider`) is the single owner of `ForegroundSessionService.update()`. Cardio calls `setCardio(...)`, the heart-rate panel calls `setHeartRatePanel(...)`, and `coachKeepAliveProvider` calls `setCoach(...)` while the coach is talking through a workout; the Android foreground service runs when any of them needs it, and GPS is only requested by cardio. The coach adds the `mediaPlayback` service type and a "Turn coach off" notification button, relayed from the service isolate via `FlutterForegroundTask.initCommunicationPort()` in `main()`. Never call `ForegroundSessionService` directly from a controller.
- **Weekly heart report**: `BuildWeeklyHeartReportUseCase` (`lib/features/heart_rate/application/`) folds the last seven days of strength-set avg/peak HR and cardio HR samples into a `WeeklyHeartReport`; `weeklyHeartReportProvider` feeds `WeeklyHeartReportScreen` at `/heart-rate/weekly-report` (nested under the `/heart-rate` route), opened from the panel's report row.
...
- **CloudSyncService**: Abstract interface with `isSupported` (false only for `NoopCloudSyncService` on desktop/web — the orchestrator refuses to sync and the settings toggle is disabled), `isAvailable()`, `uploadSnapshot()`, `downloadSnapshot()`, `deleteCloudData()`. `CloudKitSyncService` (iOS, platform channel) and `GoogleDriveSyncService` (Android, googleapis). `deleteCloudData()` waits for any in-flight `sync()` so its upload cannot land after the delete.
...
- The snapshot carries no client roster or per-row owner, so `createFromDatabase()` reads only Me's rows for client-scoped tables (other clients' data stays local); the apply guard mirrors the merge engine's tombstone-first ordering rather than comparing `updated_at` alone.
...
- **Trainer** (`lib/features/trainer/`): Coach mode. `Persona` holds ARB phrase keys rather than literal text, so all coaching copy is localised; three packs (Steady, Hype, Sergeant) in `persona_packs.dart`. `CoachingEngine` decides when to speak; `CoachBridge` and `coach_announcements.dart` wire it to the UI. Encouragement is suppressed above the clinician cap, in caution mode, and in zone 5 — the safety warning itself still speaks. `TrainerSettings.speakInBackground` (default on) keeps the coach talking with the screen off; off, `CoachBridge` drops cues while the app is hidden or paused. `ActivityDetector` + `HrEventSource` emit `ActivityDetected` (opt-in, `activityNudgesEnabled`) after 90 s of sustained effort with no session running; `ActivityNudgeListener` around the shell shows the one-tap Start workout offer, withheld above the cap, in zone 5 and in caution mode. Gated behind `Entitlement.virtualTrainer`, which is empty by default — `TrainerEventBus.emit` drops every event when unentitled.
...
- **Cardio history** (History tab, third top tab): `CardioHistoryView` (`lib/features/history/presentation/widgets/`) with a History | Progress segmented control, sport chips (`cardioSportFilterProvider`), `cardioHistoryProvider` → `BuildCardioHistoryUseCase` (sessions + start time + sport name), and `cardioProgressProvider` → pure `CardioProgress.compute()` (six calendar weeks: distance, sessions, time, weekly bars, best pace, avg HR, pace trend). Session cards open `/history/:workoutId`; the detail's cardio card draws `CardioRouteSketch` (GPS projection, no map tiles) and an HR `LineChart` from the stored recordings.
