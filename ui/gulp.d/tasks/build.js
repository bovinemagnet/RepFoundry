...
module.exports = (src, dest, preview) => () => {
  ...
  const postcssPlugins = [
    ...
    // NOTE preserve:true is REQUIRED — the RepFoundry theme switches dark/light
    // at runtime by re-binding custom properties on [data-theme]. Flattening
    // them at build time would freeze the site in dark mode.
  ...
  return merge(
    ...
    vfs
      ...
      .pipe(bundle(opts))
      .pipe(uglify({ output: { comments: /^! / } })),
    vfs.src(['css/site.css', 'css/vendor/*.css'], { ...opts, sourcemaps }).pipe(
      postcss((file) => ({ plugins: postcssPlugins, options: { file } }))
    ),
    vfs.src('font/*.{ttf,woff*(2)}', { ...opts, allowEmpty: true }),
    vfs.src('img/**/*.{gif,ico,jpg,png,svg}', opts).pipe(preview ? through() : imagemin()),
    vfs.src('helpers/*.js', opts),
    vfs.src('layouts/*.hbs', opts),
    vfs.src('partials/*.hbs', opts),
    vfs.src('static/**/*[!~]', { ...opts, base: ospath.join(src, 'static'), dot: true, allowEmpty: true })
