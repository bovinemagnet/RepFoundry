...
Paul Snow
0.0.0
:description: RepFoundry's privacy policy — what data the app stores, where it is stored, and what control you have over it.
...
[NOTE]
====
See xref:permissions.adoc[Permissions and device setup] and xref:guide/health-sync.adoc[Health sync] for more on how these permissions and toggles work in the app.
====
...
[NOTE]
====
See xref:guide/sync.adoc[Cloud sync] for how to turn sync on and off, and xref:guide/settings.adoc[Settings, import and export] for where these controls live.
====
