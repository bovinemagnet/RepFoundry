...
class WeeklyLoad {
  ...
  const WeeklyLoad({required this.weekStart, required this.setCount, required this.avgRpe, required this.load});
}
...
List<WeeklyLoad> computeTrainingLoad(List<SetData> sets) {
  ...
  return sorted.map((entry) {
    ...
    final avgRpe = setsWithRpe.isEmpty ? 0.0 : setsWithRpe.fold(0.0, (sum, s) => sum + s.rpe!) / setsWithRpe.length;
    return WeeklyLoad(weekStart: entry.key, setCount: entry.value.length, avgRpe: avgRpe, load: entry.value.length * avgRpe);
  }).toList();
}
